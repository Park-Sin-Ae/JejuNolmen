...
<script src="<%= request.getContextPath()%>/js/adminLodging.js"></script>
<style>
...
<%
List<ManagementLodgingVO>  list = (List<ManagementLodgingVO>)request.getAttribute("lodgingDetail");
		...
		<div class="col-9">
		    ...
		    <div id="roomListTable">
		      ...
		      <table class="table table-hover" style="width: 100%">
		       ...
		       <%
		       for(ManagementLodgingVO vo : list){
		       %> 
		        
		        <tr>
		        <td><%= vo.getRoom_type() %></td>
		        <td><%= vo.getRoom_no()%></td>
		        <td><%= vo.getRoom_prs() %></td>
		        <td><input type="button" value="객실 삭제" class="btn btn-outline-danger"></td> 
		        </tr>
		      ...
		      <div class="text-center mt-5">
		        <a href="/Team1_project/lodging/lodging.jsp" class="btn btn-outline-success"> 목록으로 돌아가기</a>
		    </div>
		    </div>
		</div>
		</div>
