...
<%
	NoticeVO detailvo = (NoticeVO)request.getAttribute("notice");
%>
<body>
<!-- header str -->
...
<div class="container">
	<div class="row">
		...
		<div class="col-9">
			<div class="mt-5">
				<h4> <%= detailvo.getNotice_title() %> </h4>
				<p class="text-muted"><%=detailvo.getNotice_date() %> </p>
				<hr>
			</div>
			<div><%= detailvo.getNotice_cont().replace("\n", "<br>").replaceAll("\\.\\s+", ".<br>") %></div>
				<div class="text-center mt-5">
					<a href="/Team1_project/notice/notice.jsp" class="btn btn-outline-success"> 목록으로 돌아가기</a>
				</div>
		</div>
	</div>
</div>
