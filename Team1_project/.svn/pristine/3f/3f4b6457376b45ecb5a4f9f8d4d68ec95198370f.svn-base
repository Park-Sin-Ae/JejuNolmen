...
<!-- header end -->
	<%
		// 이벤트 상세 정보를 가져오는 로직이 필요. 
	//     String eventImgPath = getServletContext().getRealPath("/") + "uploadFiles";
	EventVO event = (EventVO) request.getAttribute("eventDetail");
	%>
<div class="container">
	<div class="row">
		<div class="col-3">
		...
		<div class="col-9">
			...
			<h3>
		쿠폰 코드
		<%=event.getCnp_code() %></h3>
	<!-- 이미지 파일이 실제로 저장된 경로를 img 태그의 src 속성에 설정 -->

	<img
		src="<%=request.getContextPath()%>/imageView.do?file=<%=event.getEvnt_img_path()%>"
		alt='이벤트이미지'></img>;
	<div class="text-center mt-5">
		<a href="/Team1_project/event/event.jsp" class="btn btn-outline-success"> 목록으로 돌아가기</a>	
	</div>
		</div>
	</div>
</div>
